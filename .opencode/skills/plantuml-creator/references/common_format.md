...
When documenting real codebases, use comments to reference source files and line numbers. This creates traceability between diagrams and implementation.

```puml
@startuml
box "Authentication Module" /'source: @/src/auth'/
  component "JWT Validator" as JWT /'source: @/src/auth/jwt.js:45'/
  component "Password Hasher" as PH /'source: @/src/auth/password.js:78'/
end box

box "API Layer" /'source: @/src/api'/
  [UserController] as UC /'source: @/src/api/controllers/user.js:32'/
  [AuthMiddleware] as AM /'source: @/src/api/middleware/auth.js:56'/
end box

JWT -> AM : Validate token
AM -> UC : Forward request
@enduml
```
...
- Include line numbers when referencing specific functions/methods (`file.js:line`)
- Apply to participants, components, boxes, and other diagram elements
- Maintain consistent path format throughout the diagram
