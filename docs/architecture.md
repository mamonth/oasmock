...
- **Async mocking management API**: `/_mock/async/{messages,consumers}` plus `DELETE /_mock/async/consumers/{connectionId}` (canonical, protocol-neutral resource model), `/_mock/events` (name-based fire), `/_mock/examples` (unified sync/async injection with strict oneOf validation and runtime `match`/`interval`/`delay`, plus `DELETE /_mock/examples/{id}`), `/_mock/stream` (management WebSocket notifications). The removed `/_mock/ws/*`, `/_mock/events/fire`, `/_mock/async/push` and `/_mock/async/disconnect` surface answers plain 404 like any unknown route.
