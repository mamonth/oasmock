...
A server-side event bus that decouples producers from consumers: an OpenAPI response example fires a named event (`x-event-trigger`), and AsyncAPI message examples subscribe to events (`x-send-events`) to emit messages into a channel's consumers — without the two models referencing each other. Event payloads are available to consumer templates via the `{$event.*}` data source.
