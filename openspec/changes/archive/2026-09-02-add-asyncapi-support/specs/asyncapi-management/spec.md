...
Management API surface for driving async mocking at runtime: pushing message examples to connected consumers (immediately or after a delay), targeting individual consumers or broadcasting, listing live connections and open streams, firing events on the event bus, and injecting with runtime expressions — complementing the static spec-driven behavior of `asyncapi-protocols`, `asyncapi-templating`, and `event-driver`.
