...
Automatic detection and loading of AsyncAPI 3.0.0/3.1.0 specifications alongside OpenAPI specifications, so the mock server can consume event-driven API descriptions with the same `--from`/`schemas` workflow used for OpenAPI.
