...
The CLI SHALL read configuration from a `.oasmock.yaml` file in the current working directory (or user home directory). The configuration values SHALL be overridden by environment variables and command-line arguments according to precedence rules.

#### Scenario RS.YAML.1: Config file present with valid YAML
...
#### Scenario RS.YAML.2: Config file missing
...
#### Scenario RS.YAML.3: Config file malformed
...
#### Scenario RS.YAML.4: Precedence - CLI flag overrides config file
...
#### Scenario RS.YAML.5: Precedence - environment variable overrides config file
...
The YAML configuration file SHALL use simplified schema configuration keys: `schema` (single string) for one schema, `schemas` (list) for multiple schemas. Each element in `schemas` SHALL be either a string (schema path) or an object with `src` and optional `prefix`. Other options SHALL use kebab‑case keys matching CLI flag names (`port`, `delay`, `verbose`, `nocors`, `history‑size`, `no‑control‑api`).

#### Scenario RS.YAML.6: Valid YAML structure
- **WHEN** a `.oasmock.yaml` file contains keys `schema` (or `schemas`), `port`, `verbose`, `nocors`, `delay`, `history‑size`, `no‑control‑api` with appropriate values
- **THEN** the CLI parses the file successfully and applies the values

#### Scenario RS.YAML.7: Single schema configuration
...
#### Scenario RS.YAML.8: Multiple schemas configuration
- **WHEN** a `.oasmock.yaml` file contains:
  ```yaml
  schemas:
...
#### Scenario RS.YAML.9: Invalid schema configuration (both schema and schemas)
...
#### Scenario RS.YAML.10: Invalid schemas list element
- **WHEN** a `.oasmock.yaml` file contains a `schemas` list with an element that is neither a string nor an object with `src`
- **THEN** the CLI reports an error and exits with code 2 (invalid command-line arguments)

#### Scenario RS.YAML.11: CLI flag overrides YAML schema configuration
...
#### Scenario RS.YAML.12: CLI flag overrides YAML schemas configuration
