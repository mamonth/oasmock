...
func (s *Server) fireConnectBuiltIn(channel string, info ConsumerInfo) {
	...
	if !s.eventDriver.hasSubscribers("connect", prefix) {
		return
	}
	payload := map[string]any{
		"connectionId": info.ConnectionID,
	}
	s.eventDriver.fireTargeted("connect", payload, prefix, info)
}
...
func (s *Server) fireReceiveBuiltIn(channel string, in InboundMessage, prefix string) {
	if !s.eventDriver.hasSubscribers("receive", prefix) {
		return
	}
	payload := map[string]any{}
	var parsed any
	...
	s.eventDriver.fire("receive", payload, prefix, false, nil)
}
